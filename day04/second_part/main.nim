...
proc solution(filename: string): int =
    ...
    for n in 0 ..< len(numbers):
        for i in 0 ..< len(tables):
            tables[i].markNumber(numbers[n])

        tables = tables.filterIt(not it.won)

        if len(tables) == 1:
            var index = n
            while not tables[0].won:
                inc index
                tables[0].markNumber(numbers[index])
            return tables[0].sumUnmarked * numbers[index]
